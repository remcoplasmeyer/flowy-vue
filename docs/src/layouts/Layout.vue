...
q-layout.doc-layout(view="lHh LpR lff", @scroll="onScroll")
  q-header.header(elevated)
    q-toolbar
      q-btn.q-mr-sm(flat, dense, round, @click="toggleLeftDrawer", aria-label="Menu", v-if="leftDrawerEnabled")
        q-icon(:name="mdiMenu", size="sm")

      q-toolbar-title(shrink, :style="{overflow: 'visible'}", class="q-pa-none flex items-center")
        img(src="~/assets/logo.svg", style="max-height:30px;", class="q-mr-md")
        | Flowy Vue
        span(class='text-caption q-ml-sm', style="opacity:0.4;") {{ packageJSON.version }}
...
import packageJSON from '@hipsjs/flowy-vue/package.json'
